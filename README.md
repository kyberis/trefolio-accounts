...
- **Lookup:** `findUserByEmail` matches **`lower(email)`** so mixed-case rows still receive reset/verification flows. **Google-only** accounts have no `password_hash`; forgot-password intentionally sends nothing (same generic success as unknown emails).
